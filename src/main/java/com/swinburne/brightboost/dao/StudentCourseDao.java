...
public class StudentCourseDao {
	...
	public List<StudentCourse> findAll() {
		String sql = "select tc.id as id, tc.student_id as studentId, t.first_name as firstName, t.last_name as lastName, tc.course_id as courseId, c.course_name as courseName from student_course tc left join student t on tc.student_id = t.id left join course c on tc.course_id = c.id order by created_time";
	...
	public Integer save(StudentCourse tc) {
		...
		if (tc.getId() != null) {
			...
			sql = "insert into student_course(student_id , course_id , calendar, created_time) "
					+ "values(?,?,?,?)";
			return jdbcTemplate.update(sql, tc.getStudentId(), tc.getCourseId(), tc.getCalendar(),
